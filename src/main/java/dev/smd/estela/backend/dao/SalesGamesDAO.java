...
public class SalesGamesDAO {
    ...
    public Boolean save(SaleGame item) {
        ...
        String sql = "INSERT INTO sales_games (sale_id, game_id, price) VALUES (?, ?, ?)";
        try {
    ...
    public ArrayList<SaleGame> findBySaleId(Long saleId) {
        ArrayList<SaleGame> resultado = new ArrayList<>();
        try {
            Class.forName(DB_DRIVER);
            try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD); PreparedStatement preparedStatement = connection.prepareStatement("SELECT sale_id, game_id, price FROM sales_games WHERE sale_id = ?")) {
